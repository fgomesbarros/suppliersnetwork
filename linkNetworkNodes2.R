...
linkNetworkNodes2 <- function(quantileAgencies = 0,
                              quantileSuppliers = 0,
                              ...
                              suppliersNetFile = "suppliersNetwork2.gdf",
                              agenciesNetFile = "agenciesNetwork2.gdf") {
    ...
    payments <- payments %>%
        select(codigo_orgao, cnpj, total) %>%
        inner_join(agencies) %>%
    ...
    paymentsXtab <- xtabs(total ~ cnpj + codigo_orgao, payments)
    
    # Calculates weights
    agenciesNetwork <- t(paymentsXtab) %*% (paymentsXtab)
    suppliersNetwork <- paymentsXtab %*% t(paymentsXtab)
